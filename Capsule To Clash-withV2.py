...
def getNodeV(link):  # 从v2ray链接中得到节点信息
    ...
    #print(str(s1,'utf-8').split(',')[10].split('"')[3])
    name=str(s1,'utf-8').split(',')[9].split('"')[3]
    port = str(s1,'utf-8').split(',')[4].split('"')[2].split(':')[1]  # 端口
    server = str(s1,'utf-8').split(',')[3].split('"')[3]  # 地址
    uuid = str(s1,'utf-8').split(',')[10].split('"')[3]  # uuid
    alterId = str(s1,'utf-8').split(',')[5].split('"')[2].split(':')[1]  # alterid
    cipher = str(s1,'utf-8').split(',')[7].split('"')[3]  # 加密
